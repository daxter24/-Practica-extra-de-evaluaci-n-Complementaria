...
System.out.println("Cual es la posicion donde quieres actualizar el interprete");
	...
	static void RegistraInfo() throws IOException {
		  if(!c.isFree()) {
			  ...
			  for(int i=0;i<c.getSize();i++) {
				  rasf.writeUTF(c.getComposicion(i).getTituloComposicion());
				  rasf.writeUTF(c.getComposicion(i).getGenero());
				  rasf.writeUTF(c.getComposicion(i).getInterpretes());
				  rasf.writeInt(c.getComposicion(i).getDuracionMin());
				  rasf.writeInt(c.getComposicion(i).getDuracionSeg());
				  
				  rasf.writeUTF(c.getComposicion(i).getFechaRegistro().toString());
				  if(c.getComposicion(i).getFechaRegistro()==null)
					  rasf.writeUTF("null \n");
				  else
				  rasf.writeUTF(c.getComposicion(i).getFechaRegistro().toString()+"\n");
				 
				  rasf.writeUTF(c.getComposicion(i).getFechaEstreno().toString());
